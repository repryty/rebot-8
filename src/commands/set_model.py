...
async def execute(ctx: CommandContext):
    if ctx.args:
        ...
        with sqlite3.connect("data/database.db", timeout=10.0) as conn:
            ...
            cursor.execute(
                "UPDATE chat_sessions SET model=? WHERE session_id=?", (ctx.args[0], ctx.guild.id)
            )
            conn.commit()
        return
            ...
            if any(major_model in model.name for major_model in major_models):
                models_list.append(
                    discord.SelectOption(
                        label=model.display_name, value=model.name, description=model.name
                    )
                )
    ...
    async def select_callback(interaction: discord.Interaction):
        await interaction.response.send_message(
            f"모델이 `{genai_client.models.get(model=select.values[0]).display_name}`으로 변경되었습니다!"
        )
        with sqlite3.connect("data/database.db", timeout=10.0) as conn:
            cursor = conn.cursor()

            cursor.execute(
                "UPDATE chat_sessions SET model=? WHERE session_id=?",
