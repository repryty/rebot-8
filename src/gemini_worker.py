    ...
    async def worker_loop(self):
        while self.running:
            ...
            with sqlite3.connect("data/database.db", timeout=10.0) as conn:
                ...
                cursor = conn.cursor()
                cursor.execute("SELECT message_content, attachments_list, role FROM chat_messages WHERE session_id = ? ORDER BY timestamp ASC", (task["guild_id"], ))
            ...
            for message in messages:
                ...
                chat_history.append(types.Content(
                    role=message["role"],
                    parts=[
                        types.Part(text=message["message_content"])
            ...
            response = genai_client.models.generate_content_stream(
                ...
                config=types.GenerateContentConfig(
                    ...
                    thinking_config=types.ThinkingConfig(thinking_budget=(900*thinking), include_thoughts=True) if "2.5" in model else None,
            ...
            for chunk in response:
                ...
                if part.thought:
                    ...
                    if time.time() - last_update_time > 0.5: # 이 또한 너무 잦은 업데이트를 방지합니다.
                        await msg.edit(content=output if output else "<a:loading:1264015095223287878> "+part.text.split("\n")[0])
                        last_update_time = time.time()
                else:
                    ...
                    if time.time() - last_update_time > update_interval:
                        await msg.edit(content=apply_custom_emoji(output if output else "<a:loading:1264015095223287878>"))
                        last_update_time = time.time()
            ...
            final_content = apply_custom_emoji(output + (("\n-# 다음 웹 사이트에서 참조됨: " + ", ".join(list(set(grounding_data)))) if grounding_data else ""))
            await msg.edit(content=final_content if final_content.strip() else "생성된 콘텐츠가 없습니다.")
            all_output+=output
